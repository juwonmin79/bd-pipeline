...
export default function App() {
  ...
  // last_seen: 로그인 시 이전 값 읽고 → 현재 시간으로 업데이트
  useEffect(() => {
    if (!session) return
    const updateLastSeen = async () => {
      const userId = session.user.id
      // 이전 last_seen 읽기
      const { data } = await supabase.from('users').select('last_seen').eq('id', userId).single()
    ...
    <div style={{ display:'flex', height:'100vh', background: bg, fontFamily:"'Geist', sans-serif" }}>
      ...
      <div style={{ width: collapsed ? 48 : 200, minWidth: collapsed ? 48 : 200, transition:'width 0.2s ease, min-width 0.2s ease', background: sidebarBg, borderRight:`1px solid ${border}`, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        <nav style={{ flex:1, padding:'8px 0', overflowY:'auto' }}>
          {GEARS.map(g => (
            <div key={g.key}>
              <div onClick={() => !g.locked && setGear(g.key)} style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 12px', cursor: g.locked ? 'default' : 'pointer', background: gear===g.key ? (dk?'#1a1a1a':'#f5f3ff') : 'transparent', borderRight: gear===g.key ? '2px solid #7c3aed' : '2px solid transparent' }}>
      ...
      <div style={{ flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
        {gear === 'planning' && <Planning tab={planningTab} darkMode={dk} session={session} lastSeen={lastSeen} />}
